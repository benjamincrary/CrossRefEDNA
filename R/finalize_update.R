...
finalize_update <- function(destination) {
  ...
  if(status == "fail") {
    ...
    file.copy(paste0(destination, "Input.RDS"), paste0(destination, "Input_backup.RDS"), overwrite = TRUE)
    file.copy(paste0(destination, "AllPubs.RDS"), paste0(destination, "AllPubs_backup.RDS"), overwrite = TRUE)
  }
